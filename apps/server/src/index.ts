...
const server = http.createServer((req, res) => {
  if (req.method === "GET" && req.url === "/health") {
    ...
    res.end(JSON.stringify({ ok: true, service: "tfclaw-relay", time: new Date().toISOString() }));
    return;
  }
...
wss.on("connection", (ws, req) => {
  ...
  console.log(`[connected] role=${meta.role} token=${meta.token} peer=${peer}`);
});
...
server.on("upgrade", (req, socket, head) => {
  try {
    ...
    if ((roleRaw !== "agent" && roleRaw !== "client") || !token) {
...
server.listen(RELAY_PORT, RELAY_HOST, () => {
  console.log(`TFClaw relay server listening on ${RELAY_HOST}:${RELAY_PORT}`);
});
